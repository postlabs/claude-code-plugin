...
2. **On failure, repair — do not hand it back:** `recall` the donut, read
   `error_code` first (it names the bug class), then fix the workspace source
   and re-register:
...
Common `error_code`s: `tool_failed` (kit Python raised — fix, reload),
`output_shape_mismatch` (`to:` mapping or return shape wrong),

...
`dough-creator/` is ONE plugin folder that serves two harnesses from one copy
...
2. how the pipeline steps are named — Claude has real slash commands
   (`/dough-creator:test`, …), Codex has none and refers to them as prose
   ("the test step", …)
...
```
# 1. edit a shared skill — ONLY ever in dough-creator/skills-src/
# 2. regenerate both harnesses
python dough-creator/skills-src/sync_skills.py

# CI / pre-commit: fail if a generated SKILL.md is stale
python dough-creator/skills-src/sync_skills.py --check
```
...
`skills-codex/dough-creator-codex/SKILL.md` is Codex-only (it replaces Claude's

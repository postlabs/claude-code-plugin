...
```
python ${CODEX_PLUGIN_ROOT}/scripts/dough_publish.py publish <dough_dir> [--draft]
```
...
3. Static-validate:
   `python ${CODEX_PLUGIN_ROOT}/scripts/offline_validate.py <dough_dir>` —
...
the repair loop, "done = a real bake ran green" — is the Dough Creator test
step, not this step.** Author so it WILL bake; do not bake here.

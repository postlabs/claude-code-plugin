...
3. **A RESERVED vendor in the kit id.** A kit you author here is THIRD-PARTY —
   ...
   truth: the Dough Creator test step registers the kit against the live Toast and rejects a reserved
   ...
   the test step confirm. **Why a dotted id is a silent trap:** `postlab.law.korea`
   ...
   a separate, later, manual step — never the output of the build step.
...
python ${CODEX_PLUGIN_ROOT}/scripts/tool_runner.py <kit_dir> <tool_symbol> --inputs '<json kwargs>'
```
...
- Author connect.py / rule 3.4 (`auth.category: local`) correctly NOW even
  ...
  (the Dough Creator test step), and a bad one fails the first install. Likewise a bad verb or a
  `model:` ref to a missing type is a LOAD-time check: `offline_validate`
  flags what it can, but some only surface when the test step installs the kit, so
  double-check the verb against the closed list above.

## Binding + baking is the test step, not here

`kit_lifecycle.py install` / `reload` and the bake-and-repair loop live in
the Dough Creator test step. There, `install` copies the cwd dir into the active
...
reference, and install/bind failure troubleshooting all live in the Dough Creator
test step — that is where bind/reload/bake run.
...
and going through review — is a separate later step, NOT part of the build step.
Do not do it inside a build run.

...
3. **A RESERVED vendor in the kit id.** A kit you author here is THIRD-PARTY —
   ...
   truth: the test step registers the kit against the live Toast and rejects a reserved
...
```yaml
path: my_kit              # single-segment, distinctive & original — not a reserved/provider name; the test step enforces it (killer #3)
...
- Author connect.py / rule 3.4 (`auth.category: local`) correctly NOW even
  ...
  (the test step), and a bad one fails the first install. Likewise a bad verb or a
...
the test step. There, `install` copies the cwd dir into the active
...
the test step — that is where bind/reload/bake run.
...
Do not do it during the build step.
